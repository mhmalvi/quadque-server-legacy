...
export default {
  ...
  methods: {
    ...
    save() {
      let url;
      if (this.is_editing) {
        url = `/admin/service-desktop/update/`;
      } else {
        url = `/admin/service-desktop/store`;
      }
    ...
    editList(list_id) {
      ...
      axios
        .get(`/admin/service-desktop/edit/${this.temporary_id}`)
        .then((response) => {
          console.log(response);
    ...
    destroyList(list_id) {
      axios.get(`/admin/service-desktop/delete/${list_id}`).then((response) => {
        // this.success = response.data.success;
        this.fetchAll();
        this.$swal.fire({
          icon: "error",
